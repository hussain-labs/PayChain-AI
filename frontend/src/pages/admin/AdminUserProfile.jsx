        ...
        <div className="dashboard-content-wrapper">
          ...
          {(loading || !user) ? (
            ...
            <div style={{ padding: '1.5rem 0', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              {/* Back button */}
              <div>
                <Link to="/admin/users" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.45rem', color: 'var(--primary)', fontSize: '0.88rem', fontWeight: 600, textDecoration: 'none', background: 'rgba(99,102,241,0.08)', padding: '0.45rem 1rem', borderRadius: '10px', border: '1px solid rgba(99,102,241,0.2)', transition: 'all 0.2s' }}>
                  <i className='bx bx-left-arrow-alt' style={{ fontSize: '1.1rem' }} /> Back to Users
                </Link>
              </div>

            {/* Profile Hero Card */}
            <div style={{ background: 'linear-gradient(135deg, var(--primary) 0%, #8b5cf6 60%, #a78bfa 100%)', borderRadius: '22px', padding: '2.5rem', position: 'relative', overflow: 'hidden', boxShadow: '0 12px 40px rgba(99,102,241,0.35)' }}>
              {/* bg decoration */}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.75rem', position: 'relative', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative', flexShrink: 0 }}>
                  <img src={avatarSrc} alt={user?.name} style={{ width: '96px', height: '96px', borderRadius: '50%', border: '4px solid rgba(255,255,255,0.4)', objectFit: 'cover', display: 'block' }} />
                  <div style={{ position: 'absolute', bottom: 0, right: 0, width: '22px', height: '22px', borderRadius: '50%', background: user?.isActive !== false ? '#10b981' : '#ef4444', border: '3px solid rgba(255,255,255,0.6)' }} />
                </div>
                <div style={{ flex: 1, minWidth: '180px' }}>
                  <h2 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#fff', margin: '0 0 0.25rem' }}>{user?.name}</h2>
                  <p style={{ color: 'rgba(255,255,255,0.8)', margin: '0 0 0.75rem', fontSize: '0.92rem' }}>{user?.email}</p>
                ...
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                  {[
                    ...
                    { label: 'Plan Limit', value: planMax ? `${basePlanMax}/day ${user?.bonusTransactions ? `(+${user.bonusTransactions} Bonus)` : ''}` : '∞', icon: 'bx-shield' },
                  ].map(s => (
                    <div key={s.label} style={{ background: 'rgba(255,255,255,0.12)', borderRadius: '14px', padding: '0.85rem 1.1rem', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.2)', textAlign: 'center', minWidth: '90px' }}>
                      <i className={`bx ${s.icon}`} style={{ fontSize: '1.2rem', color: 'rgba(255,255,255,0.7)', display: 'block', marginBottom: '0.3rem' }} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.25rem' }}>
              ...
              <SectionCard title="Edit Profile Information" icon="bx-user-edit">
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
                  {[
                    { label: 'Full Name', key: 'name', type: 'text', icon: 'bx-user', placeholder: 'Full name' },
                  ...
                  <button
                    ...
                    style={{ marginTop: '0.25rem', padding: '0.8rem', background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: '12px', cursor: saving.info ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', opacity: saving.info ? 0.7 : 1, transition: 'opacity 0.2s' }}
                  >
